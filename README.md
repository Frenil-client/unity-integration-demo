...
**보상** — 쌓인 기록(강화 리포트 + 임무 보상)을 팝업으로 보여줍니다. **여는 순간이 곧 확인**이라
그 버튼의 점이 바로 꺼집니다. 팝업은 오른쪽 위 `X` 또는 **바깥 어두운 영역을 클릭**하면 닫히고,
닫을 때 목록이 비워집니다. 닫는 방법이 둘이지만 ViewModel의 같은 메서드로 들어가므로
"어떤 경로로 닫으면 목록이 안 지워진다" 같은 어긋남이 생기지 않습니다.

**임무** — 완료된 일일 임무를 수령하면 **전 캐릭터에 축복이 걸립니다**(공격력 +10%, 수령할 때마다 중첩).
카드의 공격력과 전투력이 즉시 오르고, 수령 기록이 보상 목록에 남아 보상 버튼의 점이 켜집니다.
임무 버튼의 점은 꺼지고요 — 한 동작이 세 가지를 동시에 움직이는 지점입니다.

### 축복은 강화와 다르게 동작합니다

같은 "공격력이 오른다"인데 내부 경로가 다릅니다.

| | 강화 | 축복 |
|---|---|---|
| API | `AddBaseValue` | `AddModifier` |
| 성격 | 영구 | 붙였다 뗄 수 있음 |
| 되돌리기 | 안 됨 | 목록에서 빼고 기본값부터 재계산 |

축복을 중첩할 때 **이전 축복을 걷어내고 새 배율로 다시 겁니다.** 그래서 2중첩이
`620 × 1.1 × 1.1 = 750`이 아니라 `620 × 1.2 = 744`가 됩니다. 나중에 소환한 캐릭터에게도
같은 축복이 자동으로 적용되고요.

이게 stat-system이 v3.0.0에서 도입한 모디파이어 스택입니다. 기본값에 직접 더하고 빼는 방식은
상한 클램프와 만나면 값이 어긋나는데(더할 때 잘리고 뺄 때는 전액이 빠짐), 모디파이어는
원리적으로 그 문제가 없습니다.
...
├─ Character ● 2      └ CharacterLevelUp  미확인 보상 기록   → 보상 버튼
...
`Window ▸ General ▸ Test Runner ▸ EditMode ▸ Run All` — 21종.
...
독립적으로 오르내리는지, 축복이 기본값을 건드리지 않고 중첩 시 곱해지지 않는지,
`Dispose`가 `Stat.Changed` 구독을 실제로 푸는지.
